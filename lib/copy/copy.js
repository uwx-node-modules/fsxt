...
import { stat as _stat, lstat, unlink, copyFile as _copyFile, chmod, mkdir, readdir, readlink, symlink } from 'graceful-fs';
...
export default function copy(src, dest, opts, cb) {
    ...
    checkPaths(src, dest, 'copy', opts, (err, stats) => {
        if (err) return cb(err);
        const { srcStat, destStat } = stats;
        checkParentPaths(src, srcStat, dest, 'copy', err => {
            if (err) return cb(err);
...
function getStats(destStat, src, dest, opts, cb) {
    const stat = opts.dereference ? _stat : lstat;
...
function copyDirItem(items, item, src, dest, opts, cb) {
    ...
    runFilter(srcItem, destItem, opts, (err, include) => {
        ...
        checkPaths(srcItem, destItem, 'copy', opts, (err, stats) => {
            if (err) return cb(err);
        ...
        if (!destStat) {
            ...
            readlink(dest, (err, resolvedDest) => {
                if (err) {
                    ...
                    if (err.code === 'EINVAL' || err.code === 'UNKNOWN') return symlink(resolvedSrc, dest, cb);
                    return cb(err);
                }
                if (opts.dereference) {
